...
// DEFAULT_PIN is intentionally empty — accounts without a configured PIN
// cannot authenticate. Set 'defaultPin' in settings.json before use.
const DEFAULT_PIN     = '';               // No fallback PIN — must be configured in settings.json

// File types eligible for compression
const COMPRESSIBLE = new Set(['.html', '.css', '.js', '.json', '.svg', '.xml']);
...
createServer(async (req, res) => {
  ...
  try {
    ...
    // ── Cache-Control: differentiated by file type ──────────────────────────
    let cacheControl;
    if (ext === '.html') {
      cacheControl = 'public, max-age=0, must-revalidate';
    } else if (['.png','.jpg','.jpeg','.gif','.webp','.svg','.ico','.mp4','.woff2','.woff','.ttf','.pdf'].includes(ext)) {
    ...
    if (ext === '.html') {
      ...
      headers['Content-Security-Policy'] =
        ...
        "img-src 'self' data: https://placehold.co https://finxtra.ca; " +
        "media-src 'self'; " +
        "connect-src 'self' https://formsubmit.co https://ipapi.co; " +
        "frame-ancestors 'none'; " +
        "upgrade-insecure-requests;";
    }

    // ── GZIP / Brotli compression for text-based files ───────────────────────
    let responseData = data;
    if (COMPRESSIBLE.has(ext)) {
      const ae = req.headers['accept-encoding'] || '';
      if (ae.includes('br')) {
        responseData = brotliCompressSync(data, {
          params: { [zlibConstants.BROTLI_PARAM_QUALITY]: 4 }
        });
        headers['Content-Encoding'] = 'br';
      } else if (ae.includes('gzip')) {
        responseData = gzipSync(data);
